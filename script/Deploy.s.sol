...
/**
 * @title DeployILInsurance
 * @notice Deploys ILInsurance deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
 ...
 */
contract DeployILInsurance is Script {
    uint160 internal constant FLAGS = uint160(Hooks.AFTER_INITIALIZE_FLAG | Hooks.AFTER_ADD_LIQUIDITY_FLAG | Hooks.AFTER_REMOVE_LIQUIDITY_FLAG | Hooks.AFTER_SWAP_FLAG | Hooks.AFTER_SWAP_RETURNS_DELTA_FLAG);

    /// @notice Set before deploying: this is part of what the pool is, and it cannot change afterwards.
    uint24 public _premiumBps;

    /// @notice Set before deploying: this is part of what the pool is, and it cannot change afterwards.
    uint32 public _coverageBps;

    /// @notice Set before deploying: this is part of what the pool is, and it cannot change afterwards.
    uint32 public _fullCoverAfter;
    ...
    function run() external {
        ...
        bytes memory creationCode = type(ILInsuranceHook).creationCode;
        bytes memory constructorArgs = abi.encode(manager, _premiumBps, _coverageBps, _fullCoverAfter);
        ...
        console2.log("ILInsurance ", predicted);
        ...
        vm.startBroadcast();
        ILInsuranceHook hook = new ILInsuranceHook{salt: salt}(manager, _premiumBps, _coverageBps, _fullCoverAfter);
        vm.stopBroadcast();
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("ILInsurance")),
